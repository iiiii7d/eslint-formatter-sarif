...
function mapLintResultToResult(lintResult) {
...
function mapRulesToReportingDescriptors(rulesMeta) {
    return Object.entries(rulesMeta).map(([ruleId, ruleMeta]) => ({
        ...
        messageStrings: ruleMeta.messages
            ? Object.fromEntries(Object.entries(ruleMeta.messages).map(([k, v]) => [k, { text: v }]))
            : undefined,
        properties: {
            type: ruleMeta.type,
...
export default async function formatter(results, context) {
    ...
    const sarifResults = results.flatMap((lintResult) => mapLintResultToResult(lintResult));
    ...
    const sarifLog = {
        ...
        properties: {
            ...
            eslintErrorCount: results.reduce((sum, result) => sum + result.errorCount, 0),
            eslintWarningCount: results.reduce((sum, result) => sum + result.warningCount, 0),
        },
    };
